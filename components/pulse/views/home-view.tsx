...
export function HomeView({ onChatOpen, onStartCheckIn, todayEntry }: HomeViewProps) {
  ...
  // Calculate streak-based discounts using Convex streak data
  const userTier = profile?.subscriptionTier || 'Starter';
  const baseDiscount = getTierDiscount(userTier);
  const streakBonus = calculateStreakBonus(convexStreak);
  const totalDiscount = baseDiscount + streakBonus;
  const nextMilestone = Math.ceil(convexStreak / 3) * 3;
  const daysToNextBonus = nextMilestone - convexStreak;
            ...
            >
              <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4 border border-white/20 mt-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-white/80 text-sm">Base ({userTier})</span>
